...
// Helper: extract base filename (without path and extension)
...
// Helper: process all collected data directives
...
int exe_first_pass(char *file_name) {
    ...
    while (fgets(line_buf, MAX_LINE_LENGTH, fp)) {
        ...
        if (is_instr(inst.opcode)) {
            ...
            // Emit code for instructions (not data/entry/extern)
            // This is where you should assemble the instruction and add it to the code array
            // For now, call a modular helper (to be implemented) that handles code emission
    ...
    // Modular: process all collected data directives
    DC = 0;
    data_count = 0;
    ...
    // Modular: get base filename for output
    char basefile[256] = {0};
